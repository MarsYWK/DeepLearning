...
data_dir='temp/cifar-10-batches-bin' # 定义数据集所在文件夹路径  
...
def read_cifar10(filename_queue):  
    ...
    # 返回从filename_queue中读取的(key, value)对，
    #key和value都是字符串类型的tensor，并且当队列中的某一个文件读完成时，该文件名会dequeue  
    ...
    *创建了一个QueueRunner对象，用于启动一个进程压数据到队列 
    ...
    if shuffle_flag == False:  
        ...
        image_batch, label_batch = tf.train.shuffle_batch(tensors=img_obj,   
                                                          ...
                                                          capacity=min_samples_in_queue+3*batch_size)                                                      
    tf.summary.image('input_image', image_batch, 6) #输出预处理后图像的summary缓存对象，用于在session中写入到事件文件中                                                      
