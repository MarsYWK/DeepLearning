...
def eval_one(summary_op, summary_writer, saver, predict_true_or_false):
    with tf.Session() as sess:
        ...
        coord = tf.train.Coordinator() #返回一个coordinator类对象，这个类实现了一个简单机制，可以用来coordinate很多线程的结果
        ...
        try:
            ...
            for queue_runner in tf.get_collection(key=tf.GraphKeys.QUEUE_RUNNERS):
                ...
                iter_num = 0
                true_test_num = 0
                #这里使用取整后的测试数据块个数，来计算测试样例的总数目，理论上这样算测试样例总数会偏大，暂时还未理解？？？
...
def evaluate():
    with tf.Graph().as_default() as g:
        img_batch, labels = input_dataset.preprocess_input_data()
        ...
        #返回要恢复的names到Variables的映射，也即一个map映射。
        #如果一个变量有moving average,就使用moving average变量名作为the restore  
        # name, 否则就使用变量名
        ...
        summary_writer = tf.summary.FileWriter(logdir='temp/event-log-test', graph=g)
